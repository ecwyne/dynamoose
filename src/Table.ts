...
const debug = Debug('dynamoose:table');
...
export default class Table {
    ...
    deleteIndex(indexName: string) {
        ...
        table.base.ddb().updateTable(params, function(err: Error, data: any) {
            debug('deleteIndex handler running');
            if (err) {
                deferred.reject(err);
            } else {
                setTimeout(function() {
                    table.waitForActive().then(function() {
    ...
    createIndex(attributes: any, indexSpec: any) {
        ...
        this.base.ddb().updateTable(params, function(err: Error, data: any) {
            if (err) {
                deferred.reject(err);
            } else {
                setTimeout(function() {
            ...
            this.describe()
                .then(function(this: any, data: any) {
                    ...
                    if (table.options.update) {
                        ...
                        Q()
                            .then(function() {
                                return indexes.delete.reduce(function(
                                    cur,
                                    next,
                                ) {
                                    return cur.then(function() {
                                        return table.deleteIndex(
                                            next.IndexName,
                                        );
                                    });
                                },
                            ...
                            .then(function() {
                                return indexes.create.reduce(function(
                                    cur,
                                    next,
                                ) {
                                    return cur.then(function() {
                                        return table.createIndex(
                                            localTableReq.AttributeDefinitions,
                                            next,
                                        );
                                    });
                                },
                        ...
                        if (
                            indexes.delete.length > 0 ||
                            indexes.create.length > 0
                        ) {
                            debug(
    ...
    waitForActive(
        ...
        var timeoutAt = Date.now() + (timeout as number);
         ...
         return deferred.resolve();
            ...
            if (Date.now() > timeoutAt) {
                return deferred.reject(
                    new Error(
                        'Wait for Active timed out after ' + timeout + ' ms.',
                    ),
                );
            }
    ...
    describeTTL(next?: Callback) {
        ...
        ddb.describeTimeToLive(params, function(
            err: Error,
            ttlDescription: any,
        ) {
            if (err) {
    ...
    updateTTL(next?: Callback) {
        ...
        if (this.schema.expires && !this.base.endpointURL) {
            this.describeTTL().then(function(ttlDesc: any) {
                ...
                var params = {
                    ...
                    TimeToLiveSpecification: {
                        AttributeName:
                            table.schema.expires &&
                            table.schema.expires.attribute,
                        Enabled: true,
                    },
                };

                var ddb = table.base.ddb();
                ddb.updateTimeToLive(params, function(err: Error) {
                    if (err) {
                        return deferred.reject(err);
                    }
                    return deferred.resolve();
    ...
    describe(next?: Callback) {
        ...
        var ddb = this.base.ddb();
        ddb.describeTable(describeTableReq, function(err: Error, data: any) {
            if (err) {
    ...
    buildTableReq(name: string, schema: Schema) {
        var attrDefs = [];

        var keyAttr: any = {};

        function addKeyAttr(attr: any) {
            if (attr) {
        ...
        for (var globalIndexName in schema.indexes.global) {
            ...
            var rangeKeyName =
                schema.indexes.global[globalIndexName].indexes[globalIndexName]
                    .rangeKey;
        ...
        var keySchema = [
            {
                AttributeName: schema.hashKey && schema.hashKey.name,
                KeyType: 'HASH',
            },
        ];
        if (schema.rangeKey) {
        ...
        if (this && this.options) {
            if (
                this.options.steamOptions &&
                this.options.steamOptions.enabled === true
            ) {
    ...
    create(next?: Callback) {
        ...
        ddb.createTable(createTableReq, function(err: Error, data: any) {
            if (err) {
    ...
    delete(next?: Callback) {
        ...
        ddb.deleteTable(deleteTableReq, function(err: Error, data: any) {
            if (err) {
...
var compareIndexes = function compareIndexes(local: any, remote: any) {
    var indexes: { [key: string]: Array<any> } = {
        delete: [],
        create: [],
    };
        ...
        for (j = 0; j < remoteIndexes.length; j++) {
            if (remoteIndexes[j].IndexName === localIndexes[i].IndexName) {
                ...
                if (!_.isEqual(remoteIndex, localIndex)) {
                    localIndex.ProvisionedThroughput =
                        localIndexes[i].ProvisionedThroughput;
