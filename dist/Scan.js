...
class Scan {
    constructor(Model, filter, options = { all: { delay: 0, max: 1 } }) {
        this.Model = Model;
    ...
    exec(next) {
        ...
        let scanReq = { TableName: '', ExclusiveStartKey: {} };
            ...
            dbClient.scan(scanReq, function (err, data) {
                if (err) {
                    ...
                    return deferred.resolve(data.Items.map(function (item) {
                        ...
                        model = new Model_1.default(item);
                        model.$__.isNew = false;
                        debug('scan parsed model', model);
                        return model;
                    }));
        ...
        if (this.filters && typeof this.filters.FilterExpression === 'string') {
            ...
            if (Object.keys(this.filters).length > 0) {
                ...
                for (const name in this.filters) {
                    ...
                    const isContains = filter.comparison === 'CONTAINS' ||
                        filter.comparison === 'NOT_CONTAINS';
                    const isListContains = isContains &&
                        filterAttr &&
                        filterAttr.type.name === 'list';
                    if (filter.values) {
                        for (var i = 0; i < filter.values.length; i++) {
                            var val = filter.values[i];
                            scanReq.ScanFilter[name].AttributeValueList.push(isListContains
        ...
        function scanSegment(segment) {
            ...
            function scanOne() {
                ...
                Model.$__.base
                    .ddb()
                    .scan(scanOneReq, function (err, data) {
                    if (err) {
        ...
        function scan() {
            ...
            Q.all(scans)
                ...
                var lastKeys = results.map((r) => r.lastKey);
    ...
    where(filter) {
        ...
        if (this.buildState) {
            this.validationError = new errors_1.ScanError('Invalid scan state; where() must follow comparison');
            return this;
        }
        if (typeof filter === 'string') {
            this.buildState = filter;
            if (this.filters[filter]) {
                this.validationError = new errors_1.ScanError('Invalid scan state; %s can only be used once', filter);
                return this;
            }
            this.filters[filter] = { name: filter };
        }
    ...
    compVal(vals, comp) {
        ...
        if (!this.buildState) {
            this.validationError = new errors_1.ScanError('Invalid scan state; %s must follow scan(), where(), or filter()', comp);
            return this;
        }
        if (permittedComparison.indexOf(comp) === -1) {
            this.validationError = new errors_1.ScanError('Invalid comparison %s', comp);
            return this;
        }
    ...
    beginsWith(val) {
        ...
        if (this.notState) {
            this.validationError = new errors_1.ScanError('Invalid scan state: beginsWith() cannot follow not()');
            return this;
        }
    ...
    in(vals) {
        ...
        if (this.notState) {
            this.validationError = new errors_1.ScanError('Invalid scan state: in() cannot follow not()');
            return this;
        }
    ...
    between(a, b) {
        ...
        if (this.notState) {
            this.validationError = new errors_1.ScanError('Invalid scan state: between() cannot follow not()');
            return this;
        }
