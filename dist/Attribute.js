...
class Attribute {
        ...
        if (this.type.name === 'map') {
            ...
            for (var subattrName in value) {
                if (this.attributes[subattrName]) {
                    throw new errors_1.SchemaError('Duplicate attribute: ' +
                        subattrName +
                        ' in ' +
                        this.name);
                }
                this.attributes[subattrName] = exports.create(schema, subattrName, value[subattrName]);
            }
        }
        else if (this.type.name === 'list') {
            if (value.type) {
            ...
            if (value === undefined && value[0] === undefined) {
                throw new errors_1.SchemaError('No object given for attribute:' + this.name);
            }
            ...
            for (var i = 0; i < value.length; i++) {
                this.attributes[i] = exports.create(schema, this.name, value[i]);
            }
        }
        if (this.options) {
    ...
    getTypeFromRawValue(value) {
        ...
        else if ((util.isArray(typeVal) && typeVal.length === 1) ||
        ...
        if (!type) {
            throw new errors_1.SchemaError('Invalid attribute type: ' + type);
        }
    ...
    getType(value) {
        if (!value) {
            throw new errors_1.SchemaError('Invalid attribute value: ' + value);
        }
        var type;
        var typeVal = value;
        if (value.type) {
    ...
    applyIndexes(indexes) {
        ...
        function applyIndex(i) {
            ...
            if (i.global) {
                ...
                if (i.throughput) {
                    ...
                    if ((!index.throughput.read || !index.throughput.write) &&
                        ...
                        throw new errors_1.SchemaError('Invalid Index throughput: ' + index.throughput);
                    }
                }
                else {
            ...
            if (attr.indexes[index.name]) {
                throw new errors_1.SchemaError('Duplicate index names: ' + index.name);
            }
    ...
    toDynamo(val, noSet, model, options) {
        ...
        if (!noSet && this.isSet) {
            if (!util.isArray(val)) {
                throw new errors_1.ValidationError('Values must be array: ' + this.name);
            }
            if (val.length === 0) {
                return null;
            }
        ...
        if (isSet) {
            ...
            if (!util.isArray(val)) {
                throw new errors_1.ValidationError('Values must be array in a `list`: ' + this.name);
            }
            var dynamoList = [];
            ...
            if (type.dynamo === 'S') {
                if (this.options.enum) {
                    if (this.options.enum.indexOf(val) === -1) {
                        throw new errors_1.ValidationError('Value must be one of : ' +
    ...
    parseDynamo(json) {
        ...
        function dedynamofy(type, isSet, json, transform, attr) {
            try {
                ...
                throw new errors_1.ParseError('Attribute "' +
                    attr.name +
                    '" of type "' +
                    type +
        ...
        switch (this.type.name) {
            ...
            case 'buffer':
                ...
                break;
            default:
                throw new errors_1.SchemaError('Invalid attribute type: ' + this.type);
        }
        if (this.get) {
            val = this.get(val);
        }
...
function createAttrDefFromDynamo(dynamoAttribute) {
    var dynamoType;
    var attrDef = {
        type: exports.lookupType(dynamoAttribute),
    };
...
exports.createUnknownAttrbuteFromDynamo = function (schema, name, dynamoAttribute) {
    var attrDef = createAttrDefFromDynamo(dynamoAttribute);
    var attr = new Attribute(schema, name, attrDef);
    return attr;
};
exports.create = function (schema, name, obj) {
    const value = obj;
    ...
    if (options.hashKey && options.rangeKey) {
        throw new errors_1.SchemaError('Cannot be both hashKey and rangeKey: ' + name);
    }
    ...
    if (attr.indexes) {
        for (var indexName in attr.indexes) {
            ...
            if (schema.indexes.global[indexName] ||
                schema.indexes.local[indexName]) {
                throw new errors_1.SchemaError('Duplicate index name: ' + indexName);
            }
            if (index.global) {
...
exports.lookupType = function (dynamoObj) {
